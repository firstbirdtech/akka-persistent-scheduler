...
object PersistentScheduler {
  ...
  def props(persistence: SchedulerPersistence)(implicit timeout: Timeout) = Props(new PersistentScheduler(persistence))

  def apply(persistence: SchedulerPersistence)(implicit timeout: Timeout): PersistentScheduler =
    new PersistentScheduler(persistence)
}

class PersistentScheduler(persistence: SchedulerPersistence)(implicit timeout: Timeout)
    extends Actor
    with AkkaScheduler {
  ...
  def addSubscription(subscription: Subscription): Future[Any] = {
  ...
  def schedulePersistenceCheckForEvents(): Cancellable = {
    scheduler.schedule(1.minute, 1.minute, self, CheckPersistenceForEvents())
  }

  def scheduleNextEventFromPersistence(): Option[Cancellable] = {
  ...
  def publishEvent(e: TimedEvent): Set[Future[Any]] = {

    persistence.delete(e.id)
    scheduleNextEventFromPersistence()

    subscriptions.filter(_.eventType == e.eventType).map { subscription =>
      subscription.subscriber ? e
    }
  }

  def addEvent(event: TimedEvent): Future[Any] = {
  ...
  def removeEventsByReference(eventType: String, reference: String): Future[Any] = {
  ...
  def findEventsByReference(eventType: String, reference: String): Future[Any] = {
