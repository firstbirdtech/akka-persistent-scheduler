...
object PersistentScheduler {
  ...
  def apply(persistence: SchedulerPersistence): PersistentScheduler =
...
class PersistentScheduler(persistence: SchedulerPersistence)
  ...
  private def schedulePersistenceCheckForEvents(): Unit = {
  ...
  private def addEvent(event: TimedEvent): Unit = {
